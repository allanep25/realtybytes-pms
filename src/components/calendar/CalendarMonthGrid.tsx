    ...
    <>
      <div className="rounded-xl border border-slate-200 bg-card shadow-sm">
        ...
        <p className="border-b border-slate-50 px-4 py-1.5 text-xs text-slate-400">
          Full month view · 7 days per row · click a booking to view details
        </p>
            ...
            >
              {weeks.flat().map((day) => (
                <div
                  key={day.dateKey}
                  className={cn(
                    "flex min-h-[140px] flex-col border-b border-r border-slate-200 p-2 last:border-r-0",
      ...
      <ReservationFormModal open={modalOpen} onClose={() => setModalOpen(false)} bookable={bookable} />
